...
void createNodeList(int numb){
  struct Nodes *temp;    // I will use them after Input the Node number 1 ...
  int num,num2, i;
  struct Nodes *nodeptr = malloc(sizeof(*nodeptr));  // Memory Allocation ...
  if(nodeptr = NULL){   // Check whether the nodeptr is NULL and if so no Memory Allocation ...
    ...
    for(i=2; i<=numb; i++){
      struct Nodes *FnNodes = malloc(sizeof(*FnNodes));
      if(FnNodes == NULL){
        printf(" Memory can not be Allocated.");
        break;
      }else{
